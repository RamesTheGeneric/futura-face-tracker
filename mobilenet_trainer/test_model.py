...
def main():
    ...
    checkpoint = torch.load('trained_models\\Mobilenet-ffhq-croped\\2021-11-26_02-16-08-522369\\checkpoint-40.pth.tar')
    model.load_state_dict(checkpoint['state_dict'])
    model.eval()


    img = io.imread("C:\\Users\\louca\\Documents\\Futurabeast\\futura-face\\old\\recorded_dataset_1637092614433\\img-470.jpg")
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    img = cv2.merge([img,img,img])
    ...
    print(res.shape)
    res = res.cpu().detach().numpy()[0].reshape((20, 2))
    for e in range(20):
        cv2.circle(img, (int(res[e][0] * config.X_RES) , int(res[e][1] * config.Y_RES)) , 1, (255, 0, 0), 2)
    # points = model.predict(images)[0]
    # for i in range(int(len(res) / 2)):
